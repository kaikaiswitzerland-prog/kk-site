...
const globalStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  @keyframes particleOrbit0 { from { transform: rotate(0deg)   translateX(90px);  } to { transform: rotate(360deg)  translateX(90px);  } }
  @keyframes particleOrbit1 { from { transform: rotate(45deg)  translateX(130px); } to { transform: rotate(405deg)  translateX(130px); } }
  @keyframes particleOrbit2 { from { transform: rotate(90deg)  translateX(68px);  } to { transform: rotate(450deg)  translateX(68px);  } }
  @keyframes particleOrbit3 { from { transform: rotate(135deg) translateX(158px); } to { transform: rotate(495deg)  translateX(158px); } }
  @keyframes particleOrbit4 { from { transform: rotate(200deg) translateX(105px); } to { transform: rotate(560deg)  translateX(105px); } }
  @keyframes particleOrbit5 { from { transform: rotate(270deg) translateX(178px); } to { transform: rotate(630deg)  translateX(178px); } }
  @keyframes particleOrbit6 { from { transform: rotate(315deg) translateX(78px);  } to { transform: rotate(675deg)  translateX(78px);  } }
  @keyframes particleOrbit7 { from { transform: rotate(160deg) translateX(145px); } to { transform: rotate(520deg)  translateX(145px); } }
`;
...
// ─── CursorShowcase — antigravité spring ─────────────────────────────────────
const SHOWCASE_SLIDES = [
  { name: "Poisson Tahitien", word: "TAHITI", price: "15.50 CHF", desc: "Thon rouge, citron vert, gingembre · sauce coco",      image: "/froid-tahitien.jpg", bg: "#06100a", accent: "#3a8a5a" },
  { name: "Poisson KaïKaï",  word: "KAÏKAÏ", price: "15.50 CHF", desc: "Thon rouge, mangue, ananas · sauce sésame",             image: "/froid-kaikai.jpg",  bg: "#0e0700", accent: "#c47a2a" },
  { name: "Poisson Haka",    word: "HAKA",   price: "16.00 CHF", desc: "Thon rouge, sauce piment · citron vert",                image: "/froid-haka.jpg",    bg: "#0f0404", accent: "#b03a2a" },
  { name: "Poisson Mokaï",   word: "MOKAÏ",  price: "17.00 CHF", desc: "Thon rouge, avocat, guacamole maison · sauce arachide", image: "/froid-mokai.jpg",   bg: "#0a0800", accent: "#c0a030" },
];

const PARTICLES = [
  { size: 5, anim: 'particleOrbit0', dur: '12s',  delay: '0s',   opacity: 0.6  },
  { size: 4, anim: 'particleOrbit1', dur: '18s',  delay: '-4s',  opacity: 0.4  },
  { size: 7, anim: 'particleOrbit2', dur: '9s',   delay: '-2s',  opacity: 0.55 },
  { size: 4, anim: 'particleOrbit3', dur: '22s',  delay: '-8s',  opacity: 0.45 },
  { size: 6, anim: 'particleOrbit4', dur: '14s',  delay: '-1s',  opacity: 0.6  },
  { size: 5, anim: 'particleOrbit5', dur: '25s',  delay: '-12s', opacity: 0.35 },
  { size: 4, anim: 'particleOrbit6', dur: '8s',   delay: '-3s',  opacity: 0.5  },
  { size: 8, anim: 'particleOrbit7', dur: '16s',  delay: '-6s',  opacity: 0.4  },
];
...
function CursorShowcase() {
  ...
  const rafRef        = React.useRef(null);
  // Spring state — never in React state, only in refs
  ...
  const slide    = SHOWCASE_SLIDES[curSlide];
  const CIRCLE_D = typeof window !== 'undefined' && window.innerWidth < 768 ? 200 : 260;

  // Init + resize: target = center
  useEffect(() => {
    const snap = () => {
  ...
  // Spring RAF loop — K=80, D=12, M=1.5 → underdamped ζ≈0.55, ~13% overshoot
  useEffect(() => {
    let last = performance.now();
    const K = 80, D = 12, M = 1.5;
    ...
    const tick = () => {
      ...
      const nvx = vx + ((K * (tx - px) - D * vx) / M) * dt;
      const nvy = vy + ((K * (ty - py) - D * vy) / M) * dt;
      const npx = px + nvx * dt;
      const npy = py + nvy * dt;

      velRef.current = { x: nvx, y: nvy };
      posRef.current = { x: npx, y: npy };

      const tf = `translate(${npx}px,${npy}px) translate(-50%,-50%)`;
      if (circleRef.current)    circleRef.current.style.transform    = tf;
      if (particlesRef.current) particlesRef.current.style.transform = tf;

      if (wordRef.current && containerRef.current) {
        const w  = containerRef.current.offsetWidth  || window.innerWidth;
        const h  = containerRef.current.offsetHeight || window.innerHeight;
        const ox = (npx - w / 2) * -0.15;
        const oy = (npy - h / 2) * -0.08;
        wordRef.current.style.transform = `translate(${ox}px,${oy}px)`;
      }
  ...
  const handleMouseLeave = React.useCallback(() => {
    ...
    if (containerRef.current) {
      const w = containerRef.current.offsetWidth;
      const h = containerRef.current.offsetHeight;
      targetRef.current = { x: w / 2, y: h * 0.45 };
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      const t = e.touches[0];
      const r = el.getBoundingClientRect();
      targetRef.current = { x: t.clientX - r.left, y: t.clientY - r.top };
    };
    const onEnd = () => {
      setCursorIn(false);
      if (containerRef.current) {
  ...
  return (
    <section
      ...
      onMouseLeave={handleMouseLeave}
    >
      {/* LAYER 0 — image fond atténuée */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        <img src={slide.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.07, filter: 'saturate(0.35)', transition: 'opacity 0.6s ease' }} draggable={false} />
      </div>

      {/* LAYER 1 — mot géant, parallaxe inverse via ref */}
      <div ref={wordRef} style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '7%', zIndex: 1, pointerEvents: 'none', willChange: 'transform' }}>
        <span style={{ fontFamily: "'Bebas Neue', Impact, 'Arial Black', sans-serif", fontSize: 'clamp(90px, 21vw, 260px)', fontWeight: 900, color: 'rgba(255,255,255,0.055)', textTransform: 'uppercase', letterSpacing: '0.1em', lineHeight: 1, whiteSpace: 'nowrap' }}>
          POISSON
        </span>
      </div>

      {/* LAYER 2 — étiquette haut-gauche */}
      <div style={{ position: 'absolute', top: '7%', left: '5%', zIndex: 5, display: 'flex', alignItems: 'center', gap: 10, opacity: sectionVisible ? 1 : 0, transform: sectionVisible ? 'none' : 'translateY(-8px)', transition: 'opacity 0.6s ease, transform 0.6s ease' }}>
        <div style={{ width: 28, height: 1, background: 'rgba(255,255,255,0.3)' }} />
        <span style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)', fontWeight: 600 }}>Plats Froids · Signature</span>
      </div>

      {/* LAYER 3 — particules dorées (suivent le cercle via même ref transform) */}
      <div ref={particlesRef} style={{ position: 'absolute', top: 0, left: 0, width: 0, height: 0, zIndex: 2, pointerEvents: 'none', willChange: 'transform', opacity: sectionVisible ? (cursorIn ? 1 : 0.5) : 0, transition: 'opacity 0.6s ease' }}>
        {PARTICLES.map((p, i) => (
          <div key={i} style={{ position: 'absolute', width: p.size, height: p.size, borderRadius: '50%', background: '#C9A96E', opacity: p.opacity, top: -p.size / 2, left: -p.size / 2, animation: `${p.anim} ${p.dur} linear ${p.delay} infinite` }} />
        ))}
      </div>

      {/* LAYER 4 — cercle révélateur (spring antigravité via ref) */}
      <div ref={circleRef} style={{ position: 'absolute', top: 0, left: 0, width: CIRCLE_D, height: CIRCLE_D, borderRadius: '50%', overflow: 'hidden', zIndex: 3, opacity: cursorIn ? 1 : 0.2, transition: 'opacity 0.5s ease', boxShadow: `0 0 0 1px rgba(255,255,255,0.1), 0 0 0 3px ${slide.accent}30, 0 24px 64px rgba(0,0,0,0.6)`, pointerEvents: 'none', willChange: 'transform' }}>
        <img src={slide.image} alt={slide.name} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scale(1.08)', transition: 'opacity 0.4s ease' }} draggable={false} />
        <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: `1px solid ${slide.accent}50` }} />
      </div>

      {/* LAYER 5 — info texte, lévitation + fade-in */}
      <div key={curSlide} style={{ position: 'absolute', bottom: '11%', left: '5%', zIndex: 5, animation: sectionVisible ? 'showcaseFadeUp 0.9s cubic-bezier(0.16,1,0.3,1) forwards' : 'none', opacity: sectionVisible ? undefined : 0 }}>
        <p style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: slide.accent, marginBottom: 10, fontWeight: 600 }}>{slide.word}</p>
        <div style={{ animation: sectionVisible ? 'levitate 3s ease-in-out infinite' : 'none' }}>
          <h2 style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 'clamp(40px, 6vw, 68px)', fontWeight: 900, lineHeight: 1, color: '#fff', marginBottom: 10, letterSpacing: '0.02em' }}>{slide.name}</h2>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', maxWidth: 300, lineHeight: 1.65, marginBottom: 14 }}>{slide.desc}</p>
          <p style={{ fontSize: 20, fontWeight: 700, color: '#fff', letterSpacing: '0.04em' }}>{slide.price}</p>
        </div>
      </div>

      {/* LAYER 6 — dots navigation */}
      <div style={{ position: 'absolute', bottom: '11%', right: '5%', zIndex: 5, display: 'flex', gap: 8, alignItems: 'center' }}>
        {SHOWCASE_SLIDES.map((_, i) => (
          <button key={i} onClick={() => setCurSlide(i)} style={{ width: i === curSlide ? 26 : 8, height: 8, borderRadius: 999, background: i === curSlide ? '#fff' : 'rgba(255,255,255,0.22)', border: 'none', padding: 0, cursor: 'pointer', transition: 'all 0.35s cubic-bezier(0.16,1,0.3,1)' }} aria-label={SHOWCASE_SLIDES[i].name} />
        ))}
      </div>
    </section>
  );
