...
// ─── CursorShowcase — section vitrine cursor-reveal ──────────────────────────
const SHOWCASE_SLIDES = [
  { name: "Poisson Tahitien", word: "TAHITI",  price: "15.50 CHF", desc: "Thon rouge, citron vert, gingembre · sauce coco",       image: "/froid-tahitien.jpg", bg: "#080f0b", accent: "#3a8a5a" },
  { name: "Poisson KaïKaï",  word: "KAÏKAÏ",  price: "15.50 CHF", desc: "Thon rouge, mangue, ananas · sauce sésame",              image: "/froid-kaikai.jpg",  bg: "#100800", accent: "#c47a2a" },
  { name: "Poisson Haka",    word: "HAKA",    price: "16.00 CHF", desc: "Thon rouge, sauce piment · citron vert",                 image: "/froid-haka.jpg",    bg: "#0f0505", accent: "#b03a2a" },
  { name: "Poisson Mokai",   word: "MOKAI",   price: "17.00 CHF", desc: "Thon rouge, avocat, guacamole maison · sauce arachide",  image: "/froid-mokai.jpg",   bg: "#0c0a00", accent: "#c0a030" },
];
...
function CursorShowcase() {
  ...
  const rafRef       = React.useRef(null);
  // Raw target coords (not lerped)
  const targetRef    = React.useRef({ x: -9999, y: -9999 });
  // Lerped current position (direct DOM, no state)
  const currentRef   = React.useRef({ x: -9999, y: -9999 });
  const circleOnRef  = React.useRef(false);

  const [curSlide, setCurSlide]   = useState(0);
  const [sectionVisible, setSectionVisible] = useState(false);

  const slide = SHOWCASE_SLIDES[curSlide];
  const CIRCLE_D = typeof window !== 'undefined' && window.innerWidth < 768 ? 200 : 280;

  // ── RAF lerp loop ────────────────────────────────────────────────────────
  useEffect(() => {
    const lerp = (a, b, t) => a + (b - a) * t;
    const T = 0.09; // spring factor

    const tick = () => {
      const { x: cx, y: cy } = currentRef.current;
      const { x: tx, y: ty } = targetRef.current;
      const nx = lerp(cx, tx, T);
      const ny = lerp(cy, ty, T);
      currentRef.current = { x: nx, y: ny };

      if (circleRef.current) {
        circleRef.current.style.transform = `translate(${nx}px,${ny}px) translate(-50%,-50%)`;
      }
      if (wordRef.current && containerRef.current) {
        const w = containerRef.current.offsetWidth  || window.innerWidth;
        const h = containerRef.current.offsetHeight || window.innerHeight;
        const ox = (nx - w / 2) * -0.055;
        const oy = (ny - h / 2) * -0.035;
  ...
  // ── IntersectionObserver — déclenche le fade-in du texte ─────────────────
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
  ...
  return (
    <section
      ...
      onMouseLeave={handleMouseLeave}
    >
      {/* LAYER 0 — image de fond très atténuée */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        <img
          src={slide.image}
          alt=""
          style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.07, filter: 'saturate(0.4)', transition: 'opacity 0.6s ease' }}
          draggable={false}
        />
      </div>

      {/* LAYER 1 — mot géant, parallaxe inverse via ref direct */}
      <div
        ref={wordRef}
        style={{
          position: 'absolute',
      ...
      {/* LAYER 2 — étiquette catégorie haut-gauche */}
      <div style={{
        position: 'absolute',
        top: '7%',
        left: '5%',
        ...
        <span style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>Plats Froids · Signature</span>
      </div>

      {/* LAYER 3 — cercle curseur (position via transform, direct DOM) */}
      <div
        ref={circleRef}
        style={{
          position: 'absolute',
          ...
          boxShadow: `0 0 0 1.5px rgba(255,255,255,0.12), 0 0 0 4px ${slide.accent}35, 0 24px 64px rgba(0,0,0,0.55)`,
          pointerEvents: 'none',
          willChange: 'transform',
        }}
      >
        <img
          src={slide.image}
          alt={slide.name}
          style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scale(1.08)', transition: 'opacity 0.4s ease' }}
          draggable={false}
        />
        {/* Ring intérieur */}
        <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: `1.5px solid ${slide.accent}60`, pointerEvents: 'none' }} />
      </div>

      {/* LAYER 4 — info texte bas-gauche, fade-in + re-animation au changement de slide */}
      <div
        key={curSlide}
        style={{
          position: 'absolute',
          ...
          left: '5%',
          zIndex: 5,
          animation: sectionVisible ? 'showcaseFadeUp 0.85s cubic-bezier(0.16,1,0.3,1) forwards' : 'none',
      ...
      >
        ...
        <p style={{ fontSize: 20, fontWeight: 700, color: '#ffffff', letterSpacing: '0.04em' }}>
          {slide.price}
        </p>
      </div>

      {/* LAYER 5 — dots navigation droite */}
      <div style={{
        position: 'absolute',
        bottom: '10%',
        right: '5%',
        ...
        {SHOWCASE_SLIDES.map((_, i) => (
          <button
            ...
            style={{
              width: i === curSlide ? 28 : 8,
              height: 8,
              borderRadius: 999,
              background: i === curSlide ? '#ffffff' : 'rgba(255,255,255,0.25)',
              border: 'none',
              padding: 0,
              cursor: 'pointer',
              transition: 'all 0.35s cubic-bezier(0.16,1,0.3,1)',
            }}
            aria-label={SHOWCASE_SLIDES[i].name}
          />
        ))}
      </div>

      {/* LAYER 6 — hint mobile "glisser le doigt" */}
      <div style={{
        position: 'absolute',
        bottom: '5%',
        left: '50%',
        ...
        alignItems: 'center',
        gap: 8,
        opacity: sectionVisible ? 0.4 : 0,
        transition: 'opacity 1s ease 0.8s',
        pointerEvents: 'none',
      }}>
        <span style={{ fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#fff' }}>Glisser pour explorer</span>
      </div>
    </section>
  );
}
